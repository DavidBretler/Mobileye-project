...
def find_tfl_lights(c_image: np.ndarray):
    ...
    """
    kernel = np.array(Image.open('../Data/images/kernel.png').convert('L'))
        ...
        for j in range(len(max_filter_image[i])):
            if max_filter_image[i][j] == fixed_image[i][j] and max_filter_image[i][j] > 1500000:
                ...
                # red_index2.append(j)
                    ...
                    green_index.append((j, i))
                # green_index2.append(j)
    return red_index, green_index
...
def show_find_tfl_lights(image_path):
    ...
    red, green = find_tfl_lights(image)

    red_x = [x[0] for x in red]
    red_y = [x[1] for x in red]

    green_x = [x[0] for x in green]
...
if __name__ == '__main__':
    ...
    """
    show_find_tfl_lights('../Data/gallery/img2.png')  # , 'cologne_000113_000019_gtFine_labelIds.png')
    plt.show(block=True)
